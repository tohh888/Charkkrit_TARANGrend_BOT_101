...
function classifyIntent(message, history = []) {
    ...
    const scheduleStrong = [
        'ตาราง', 'คาบ', 'วิชา', 'รหัสวิชา', 'ห้อง', 'กลุ่ม', 'ผู้เรียน',
        'นักเรียน', 'นักศึกษา', 'ผู้สอน', 'อาจารย์', 'ครู',
        'วันจันทร์', 'วันอังคาร', 'วันพุธ', 'วันพฤหัส', 'วันศุกร์',
        'จันทร์', 'อังคาร', 'พุธ', 'พฤหัส', 'ศุกร์',
        'วันนี้', 'พรุ่งนี้', 'มะรืน', 'กี่โมง', 'เวลาเรียน', 'เวลาสอน',
        'เรียนอะไร', 'สอนอะไร', 'เลิกเรียน', 'เริ่มเรียน', 'คาบแรก',
        'คาบสุดท้าย', 'ช่วงว่าง', 'เวลาว่าง', 'วันไหน', 'ตอนไหน', 'เมื่อไหร่',
        'กี่คาบ', 'จำนวนคาบ', 'เรียนกี่วิชา', 'สอนกี่วิชา', 'สอนกี่ครั้ง',
        'น้อยสุด', 'น้อยที่สุด', 'มากสุด', 'มากที่สุด', 'เยอะสุด', 'เยอะที่สุด',
        'เบาสุด', 'หนักสุด'
    ];
    const hasStrong = scheduleStrong.some(term => q.includes(term));
    const recent = Array.isArray(history) ? history.filter(m => m?.role === 'user' && typeof m.content === 'string').slice(-3) : [];
    const hasRecentSchedule = recent.some(m => {
        const rq = normalize(m.content);
        return scheduleStrong.some(term => rq.includes(term));
    });
    const isFollowUp = /^(แล้ว|และ|งั้น|ถ้าอย่างนั้น|อันนั้น|อันนี้|ตัวนั้น|ตัวนี้|วันนั้น|วิชานั้น|กลุ่มนั้น|ของมัน|อีกล่ะ|แล้วล่ะ|ล่ะ|อะ|หรอ|เหรอ)/.test(q)
        || /(วันนั้นล่ะ|อันนั้นล่ะ|อันนี้ล่ะ|ตัวนั้นล่ะ|แล้ว.*ล่ะ|ต่อจากนั้น|ต่อไป)/.test(q);
    if (hasStrong || (isFollowUp && hasRecentSchedule)) return 'schedule';
...
function friendlyNonScheduleAnswer(message, intent) {
    ...
    if (/(ขอบคุณ|ขอบใจ)/.test(q)) return 'ยินดีครับ 😊 ถ้ามีอะไรเกี่ยวกับตารางสอน ถามต่อได้เลยครับ';
    return 'ผมช่วยเรื่องตารางสอนและข้อมูลผู้สอนได้ครับ 😊\n\nคำถามนี้ยังไม่ชัดว่าเกี่ยวกับตารางตรงไหน ลองระบุวัน วิชา กลุ่ม ห้อง หรืออาจารย์ที่ต้องการได้เลยครับ';
}
...
8. จัดคำตอบภาษาไทยให้อ่านง่าย ใช้หัวข้อและขึ้นบรรทัดใหม่เมื่อมีหลายรายการ
9. ห้ามแสดงกระบวนการคิดภายใน`
                },
                ...history,
