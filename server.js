...
function classifyIntent(message, history = []) {
    ...
    // Intent Router: ให้ "ความหมายของคำถาม" มีน้ำหนักมากกว่าการพบคำว่า "เรียน/สอน" เพียงคำเดียว
    const strongSchedule = [
        'ตาราง', 'ตารางสอน', 'ตารางเรียน', 'คาบเรียน', 'คาบสอน',
        'รหัสวิชา', 'เวลาเรียน', 'เวลาสอน', 'ช่วงว่าง', 'เวลาว่าง',
        'คาบแรก', 'คาบสุดท้าย', 'เริ่มเรียน', 'เลิกเรียน',
        'กี่คาบ', 'จำนวนคาบ', 'นับคาบ', 'สอนกี่ครั้ง', 'เรียนกี่วิชา',
        'น้อยสุด', 'น้อยที่สุด', 'มากสุด', 'มากที่สุด',
        'เยอะสุด', 'เยอะที่สุด', 'เบาสุด', 'หนักสุด',
        'วันจันทร์', 'วันอังคาร', 'วันพุธ', 'วันพฤหัส', 'วันพฤหัสบดี', 'วันศุกร์',
        'วันนี้', 'พรุ่งนี้', 'มะรืน', 'กี่โมง', 'เมื่อไหร่', 'ตอนไหน',
        'ห้องเรียน', 'ห้องที่เรียน', 'ห้องสอน'
    ];

    const scheduleRelations = [
        'เรียนอะไร', 'เรียนวิชาอะไร', 'เรียนวิชาไหน', 'เรียนวิชาใด',
        'มีเรียนอะไร', 'มีเรียนไหม', 'มีเรียนหรือเปล่า', 'มีเรียนหรือไม่',
        'สอนอะไร', 'สอนวิชาอะไร', 'สอนวิชาไหน', 'สอนวิชาใด',
        'มีสอนอะไร', 'มีสอนไหม', 'มีสอนหรือเปล่า', 'มีสอนหรือไม่',
        'เรียนวันไหน', 'สอนวันไหน', 'เรียนกี่โมง', 'สอนกี่โมง',
        'เรียนกี่คาบ', 'สอนกี่คาบ', 'กี่วิชา', 'กี่กลุ่ม', 'กี่ห้อง',
        'ว่างช่วงไหน', 'ว่างตอนไหน', 'ว่างไหม',
        'ใครสอน', 'อาจารย์คนไหนสอน', 'ครูคนไหนสอน',
        'กลุ่มไหนเรียน', 'กลุ่มอะไรเรียน', 'แต่ละกลุ่มเรียน',
        'กลุ่มไหนมีวิชา', 'กลุ่มอะไรมีวิชา',
        'วิชาไหนเรียน', 'วิชาอะไรเรียน', 'วิชานี้เรียน',
        'ห้องไหนเรียน', 'ห้องอะไรเรียน'
    ];

    // คำถามที่มีคำว่า "เรียน/อาจารย์" แต่กำลังถามเรื่องอื่น ต้องกันออกก่อน
    const offTopic = [
        'ไม่ไปเรียน', 'ไม่เข้าเรียน', 'ขาดเรียน', 'โดดเรียน', 'หนีเรียน',
        'ไม่มาเรียน', 'ลาเรียน', 'ไปเรียนทำไม',
        'ทำไมต้องเรียน', 'เรียนไปทำไม', 'เรียนแล้วได้อะไร',
        'เรียนแล้วได้เงิน', 'ได้เงินไหม', 'เงินเดือน', 'ค่าเทอม',
        'การบ้าน', 'ข้อสอบ', 'สอบยังไง', 'คะแนน', 'เกรด',
        'ลงทะเบียน', 'สมัครเรียน', 'ค่าใช้จ่าย',
        'หล่อไหม', 'สวยไหม', 'ใจดีไหม', 'ดุไหม', 'นิสัย', 'ชอบใคร',
        'รักไหม', 'แฟน', 'กินข้าว', 'อากาศ', 'การเมือง',
        'ทำไม', 'ควรไหม', 'ดีไหม', 'เป็นอะไรไหม'
    ];

    const hasStrong = strongSchedule.some(term => q.includes(term));
    const hasRelation = scheduleRelations.some(term => q.includes(term));
    const hasDay = Boolean(findDay(q));
    const hasGroup = Boolean(findGroup(q));
    const hasSubject = Boolean(findSubject(q));
    const hasScheduleEntity = hasDay || hasGroup || hasSubject ||
        /(ตาราง|คาบ|รหัสวิชา|ห้อง|เวลาเรียน|เวลาสอน|ช่วงว่าง|เวลาว่าง)/.test(q);

    // "เรียน/สอน" เดี่ยว ๆ ไม่ถือว่าเป็นคำถามตาราง
    const hasWeakOnly = /(^|\s)(เรียน|สอน|อาจารย์|ครู)(\s|$)/.test(q);

    const hasExplicitScheduleQuestion =
        hasRelation ||
        /(?:วันนี้|พรุ่งนี้|มะรืน|วัน(?:จันทร์|อังคาร|พุธ|พฤหัส(?:บดี)?|ศุกร์)|กลุ่ม|ห้อง|วิชา|อาจารย์|ครู).{0,30}(เรียน|สอน|คาบ|ตาราง|เวลา|ว่าง)/.test(q) ||
        /(?:เรียน|สอน|คาบ|ตาราง|เวลา|ว่าง).{0,30}(วันนี้|พรุ่งนี้|มะรืน|วัน(?:จันทร์|อังคาร|พุธ|พฤหัส(?:บดี)?|ศุกร์)|กลุ่ม|ห้อง|วิชา|อาจารย์|ครู)/.test(q);

    const hasOffTopic = offTopic.some(term => q.includes(term));
    if (hasOffTopic && !hasExplicitScheduleQuestion) return 'off_topic';

    const recent = Array.isArray(history)
        ? history.filter(m => m?.role === 'user' && typeof m.content === 'string').slice(-4)
        : [];

    const recentSchedule = recent.some(m => {
        const rq = normalize(m.content);
        return strongSchedule.some(term => rq.includes(term))
            || scheduleRelations.some(term => rq.includes(term))
            || Boolean(findDay(rq))
            || Boolean(findGroup(rq))
            || Boolean(findSubject(rq));
    });

    const isFollowUp = /^(แล้ว|และ|งั้น|ถ้าอย่างนั้น|อันนั้น|อันนี้|ตัวนั้น|ตัวนี้|วันนั้น|วิชานั้น|กลุ่มนั้น|ของมัน|อีกล่ะ|แล้วล่ะ|ล่ะ|อะ|หรอ|เหรอ|ต่อไป|ต่อจากนั้น)/.test(q)
        || /(วันนั้นล่ะ|อันนั้นล่ะ|อันนี้ล่ะ|ตัวนั้นล่ะ|แล้ว.*ล่ะ|ต่อจากนั้น|ต่อไป)/.test(q);

    if (isFollowUp && recentSchedule && !hasOffTopic) return 'schedule';

    if (hasScheduleEntity && (hasExplicitScheduleQuestion || hasStrong)) return 'schedule';
    if (hasRelation || hasStrong) return 'schedule';

    // ถ้าจับได้แค่คำกว้าง ๆ หรือประโยคสั้นเกินไป ห้ามสุ่มหยิบตารางมาตอบ
    if (hasWeakOnly || q.length < 3) return 'ambiguous';

    const casual = /^(สวัสดี|หวัดดี|ดีครับ|ดีค่ะ|ขอบคุณ|ขอบใจ|โอเค|โอเคครับ|โอเคค่ะ|555|ฮ่า|ฮา|แค่เล่น|ล้อเล่น)/.test(q);
    if (casual) return 'casual';

    return 'off_topic';
}

function friendlyNonScheduleAnswer(message, intent) {
    const q = normalize(message);

    if (/^(สวัสดี|หวัดดี|ดีครับ|ดีค่ะ)/.test(q)) {
        return 'สวัสดีครับ 😊 ผมช่วยค้นและวิเคราะห์ตารางสอนได้ครับ\n\nลองถามได้ เช่น\n• พรุ่งนี้สอนอะไร\n• กลุ่ม ทค.2/1 เรียนวิชาอะไร\n• วันไหนสอนน้อยสุด\n• อาจารย์ว่างช่วงไหน';
    }

    if (/(ขอบคุณ|ขอบใจ)/.test(q)) {
        return 'ยินดีครับ 😊 ถ้ามีอะไรเกี่ยวกับตารางสอน ถามต่อได้เลยครับ';
    }

    if (intent === 'ambiguous') {
        return 'ผมยังไม่แน่ใจว่าต้องการถามเรื่องไหนครับ 😊\n\nถ้าต้องการดูตาราง ลองระบุวัน วิชา กลุ่ม ห้อง หรืออาจารย์ได้เลยครับ';
    }

    return '😄 คำถามนี้ยังไม่ใช่คำถามเกี่ยวกับตารางสอนโดยตรงครับ\n\nผมช่วยค้นตารางและวิเคราะห์ข้อมูลจากตารางได้ เช่น วันเรียน วิชา กลุ่ม ห้อง เวลา จำนวนคาบ และช่วงว่างครับ';
}
...
app.post('/api/chat', async (req, res) => {
    ...
    // แยกเจตนาก่อนค้นตาราง เพื่อป้องกันคำถามกวน/นอกเรื่อง
    // ไม่ให้ระบบหยิบข้อมูลตารางบางส่วนมาตอบเพียงเพราะมีคำว่าเรียน/สอน
    const intent = classifyIntent(userMessage, history);
    console.log('Intent:', intent, '| message:', userMessage);
    if (intent !== 'schedule') {
        const answer = friendlyNonScheduleAnswer(userMessage, intent);
        res.type('text/plain; charset=utf-8').send(answer);
        return;
    }
    ...
    try {
        ...
        for (const candidate of candidates) {
            try {
                ...
                {
                    role: 'system',
                    content: `คุณคือ AI ผู้ช่วยตารางสอนภาษาไทยของอาจารย์จักรกฤษณ์ วงศ์อาษา
เจตนาที่ผ่านการคัดกรองแล้ว: schedule
ข้อมูลอ้างอิงจากตารางจริง:
${JSON.stringify(scheduleData)}

กฎสำคัญ:
1. เข้าใจภาษาไทยแบบภาษาพูด คำย่อ การละคำ สรรพนาม และคำถามต่อเนื่องจากประโยคก่อนหน้า
2. ตอบสิ่งที่ผู้ใช้ถามให้ครบ เช่น "กลุ่มไหนเรียนอะไร" ต้องตอบทั้งกลุ่มและวิชาที่เรียน ไม่ใช่ตอบเฉพาะรายชื่อกลุ่ม
3. ห้ามเดา ห้ามสร้างวัน เวลา ห้อง วิชา กลุ่ม หรือจำนวนคาบที่ไม่มีในข้อมูล
4. คำถามคำนวณ เช่น น้อยที่สุด มากที่สุด จำนวนคาบ และเวลาสอนรวม ต้องใช้ข้อมูลทั้งหมดที่เกี่ยวข้องและตรวจผลก่อนตอบ
5. ถ้าคำถามกำกวม ให้ถามกลับสั้น ๆ แทนการสุ่มเลือกข้อมูล
6. ถ้าไม่พบข้อมูลที่ตรง ให้บอกว่าไม่พบข้อมูล และอย่าเอาข้อมูลที่ใกล้เคียงมาแทน
7. คำถามกวนหรือภาษาพูดให้เข้าใจเจตนาก่อนตอบ แต่ข้อเท็จจริงจากตารางต้องคงเดิม
8. ถ้าผู้ใช้ถามนอกเรื่องตาราง ห้ามนำข้อมูลตารางมาเติมคำตอบเอง
9. ถ้าผู้ใช้ถามกำกวมและไม่มีข้อมูลพอ ให้ถามกลับสั้น ๆ แทนการเดา
10. จัดคำตอบภาษาไทยให้อ่านง่าย ใช้หัวข้อและขึ้นบรรทัดใหม่เมื่อมีหลายรายการ
11. ห้ามแสดงกระบวนการคิดภายใน`
                },
                ...history,
